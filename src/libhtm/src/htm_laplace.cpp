...
-----------------------------------------------------------------------*/
double htm_laplace_invert(cmplex       (*F)(const cmplex &s, double a, double b),
                        const double &t,
                        const double tolerance, double a, double b)
{
 ...
 * @param b rate
 */
double htm_laplace_transform(const double &t, double a, double b)
{
 ...
 * @param b rate
 */
cmplex htm_laplace_invert_func(const cmplex &s, double a, double b)
{
  switch(test_case){
		/* scale change - gradually lower numbers*/
...
void Testhtm_laplace_invert()
{
  ...
  ofstream LAPTEST;
  LAPTEST.open("htm_laplace_invertTest.csv");
  LAPTEST<<"t,f(t) analytic,f(t) numerical, error"<<endl;
  LAPTEST.precision(12);
  for (t=0.0; t<12; t+=0.1)
  {
    F=htm_laplace_invert(htm_laplace_invert_func,t,1e-8,a,b);
    f=htm_laplace_transform(t, a, b);
...
extern "C" {
	...
	double htm_laplace(double span, double seed, double rate)
	{
		double f, F;

		F = htm_laplace_invert(htm_laplace_invert_func, span, 0.00000001, seed, rate);
		f = htm_laplace_transform(span, seed, rate);
