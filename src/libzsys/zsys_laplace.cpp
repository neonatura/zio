...
-----------------------------------------------------------------------*/
double zlaplace_invert(cmplex       (*F)(const cmplex &s, double a, double b),
                        const double &t,
 ...
 * @param b rate
 */
double zlaplace_transform(const double &t, double a, double b)
{
 ...
 * @param b rate
 */
cmplex zlaplace_invert_func(const cmplex &s, double a, double b)
{
  switch(test_case){
...
void Testzlaplace_invert()
{
  ...
  ofstream LAPTEST;
  LAPTEST.open("zlaplace_invertTest.csv");
  LAPTEST<<"t,f(t) analytic,f(t) numerical, error"<<endl;
  LAPTEST.precision(12);
  for (t=0.0; t<12; t+=0.1)
  {
    F=zlaplace_invert(zlaplace_invert_func,t,1e-8,a,b);
    f=zlaplace_transform(t, a, b);
...
extern "C" {
	...
	double zlaplace(double span, double seed, double rate)
	{
		double f, F;

		F = zlaplace_invert(zlaplace_invert_func, span, 0.00000001, seed, rate);
		f = zlaplace_transform(span, seed, rate);
