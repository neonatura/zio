...
image_t *zfont_init(uint8_t ch, uint32_t color, uint32_t size)
{
	...
	image->pixel = (uint8_t *)calloc(image->width * image->height * image->bytes_per_pixel, sizeof(uint8_t));
	if (!image->pixel) {
		free(image);
		return (NULL);
	}

	pixel8 = (uint8_t *)image->pixel;
	pixel16 = (uint16_t *)image->pixel;
	pixel32 = (uint32_t *)image->pixel;
...
static const unsigned char font_table[1275] PROGMEM = 
{
